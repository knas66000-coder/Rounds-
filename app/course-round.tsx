...
import { evaluateCalculationAnswer, evaluateLogicTraceAnswer, evaluateScenarioDecision } from "@/shared/course-activity-evaluator";
import { coursePackForId } from "@/shared/course-packs";
import type { StarterCourseActivity } from "@/shared/course-pack-activities";
import { highSchoolLevelLabel, isHighSchoolLevel } from "@/lib/high-school-store";
...
export default function CourseRoundScreen() {
  ...
  const { packId = "", review, level } = useLocalSearchParams<{ packId?: string; review?: string; level?: string }>();
  const pack = coursePackForId(packId);
  const reviewOnly = review === "1";
  const levelLabel = isHighSchoolLevel(level) ? highSchoolLevelLabel(level) : null;
  ...
  const [calculationFeedback, setCalculationFeedback] = useState<ReturnType<typeof evaluateCalculationAnswer> | null>(null);
  ...
  const chooseLogic = (option: string) => {
    if (!activity || activity.kind !== "logic_trace" || selected) return;
    const result = evaluateLogicTraceAnswer(option, activity);
    setSelected(option);
    setComplete(true);
    void updateState(recordCourseRoundOutcome(state, activityId, result.outcome));
  };

  const chooseScenario = (option: string) => {
    if (!activity || activity.kind !== "scenario" || selected) return;
    const result = evaluateScenarioDecision(option, activity);
    setSelected(option);
    setComplete(true);
    void updateState(recordCourseRoundOutcome(state, activityId, result.outcome));
  };
  ...
  const submitCalculation = () => {
    if (!activity || activity.kind !== "worked_calculation" || calculationFeedback) return;
    const result = evaluateCalculationAnswer(calculationAnswer, activity);
    setCalculationFeedback(result);
    setComplete(true);
    void updateState(recordCourseRoundOutcome(state, activityId, result.outcome));
  };
...
function CalculationRound({ activity, answer, feedback, colors, onChange, onSubmit }: { activity: Extract<StarterCourseActivity, { kind: "worked_calculation" }>; answer: string; feedback: ReturnType<typeof evaluateCalculationAnswer> | null; colors: ReturnType<typeof useColors>; onChange: (value: string) => void; onSubmit: () => void }) {
  const correct = feedback?.outcome === "correct";
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.background }]}><Text style={[styles.body, { color: colors.muted }]}>{activity.scenario}</Text>{activity.safetyNote ? <Text style={[styles.helper, { color: colors.muted }]}>{activity.safetyNote}</Text> : null}</View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text><TextInput value={answer} onChangeText={onChange} editable={!feedback} keyboardType="decimal-pad" placeholder="Enter a numeric answer" placeholderTextColor={colors.muted} style={[styles.compactInput, { borderColor: colors.border, backgroundColor: colors.background, color: colors.foreground }]} accessibilityLabel="Calculation answer" />{!feedback ? <Pressable onPress={onSubmit} accessibilityRole="button" style={({ pressed }) => [styles.nextButton, { backgroundColor: colors.primary }, pressed && styles.pressed]}><Text style={[styles.nextText, { color: colors.background }]}>Check answer</Text></Pressable> : <View style={[styles.feedback, { borderColor: correct ? colors.success : colors.warning, backgroundColor: colors.background }]}><Text style={[styles.feedbackTitle, { color: correct ? colors.success : colors.warning }]}>{correct ? "Calculation matched" : "Review the steps"}</Text><Text style={[styles.body, { color: colors.muted }]}>{feedback.feedback}</Text></View>}</View>;
}

function LogicTraceRound({ activity, selected, complete, colors, onChoose }: { activity: Extract<StarterCourseActivity, { kind: "logic_trace" }>; selected: string | null; complete: boolean; colors: ReturnType<typeof useColors>; onChoose: (option: string) => void }) {
  const result = selected ? evaluateLogicTraceAnswer(selected, activity) : null;
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.background }]}><Text style={[styles.body, { color: colors.muted }]}>{activity.scenario}</Text></View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text>{activity.options.map((option) => <Pressable key={option} disabled={complete} onPress={() => onChoose(option)} accessibilityRole="button" style={({ pressed }) => [styles.option, { borderColor: selected === option ? (result?.outcome === "correct" ? colors.success : colors.error) : colors.border, backgroundColor: colors.background }, pressed && !complete && styles.pressed]}><Text style={[styles.optionText, { color: colors.foreground }]}>{option}</Text></Pressable>)}{result ? <View style={[styles.feedback, { borderColor: result.outcome === "correct" ? colors.success : colors.warning, backgroundColor: colors.background }]}><Text style={[styles.feedbackTitle, { color: result.outcome === "correct" ? colors.success : colors.warning }]}>{result.outcome === "correct" ? "Logic matched" : "Review the rule order"}</Text><Text style={[styles.body, { color: colors.muted }]}>{result.feedback}</Text></View> : null}</View>;
}

function ScenarioRound({ activity, selected, complete, colors, onChoose }: { activity: Extract<StarterCourseActivity, { kind: "scenario" }>; selected: string | null; complete: boolean; colors: ReturnType<typeof useColors>; onChoose: (option: string) => void }) {
  const result = selected ? evaluateScenarioDecision(selected, activity) : null;
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.background }]}><Text style={[styles.body, { color: colors.muted }]}>{activity.scenario}</Text>{activity.boundaryNote ? <Text style={[styles.helper, { color: colors.muted }]}>{activity.boundaryNote}</Text> : null}</View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text>{activity.options.map((option) => <Pressable key={option} disabled={complete} onPress={() => onChoose(option)} accessibilityRole="button" style={({ pressed }) => [styles.option, { borderColor: selected === option ? (result?.outcome === "correct" ? colors.success : colors.error) : colors.border, backgroundColor: colors.background }, pressed && !complete && styles.pressed]}><Text style={[styles.optionText, { color: colors.foreground }]}>{option}</Text></Pressable>)}{result ? <View style={[styles.feedback, { borderColor: result.outcome === "correct" ? colors.success : colors.warning, backgroundColor: colors.background }]}><Text style={[styles.feedbackTitle, { color: result.outcome === "correct" ? colors.success : colors.warning }]}>{result.outcome === "correct" ? "Decision matched" : "Review the scenario"}</Text><Text style={[styles.body, { color: colors.muted }]}>{result.feedback}</Text></View> : null}</View>;
}
...
const styles = StyleSheet.create({
  content: { paddingTop: 19, paddingBottom: 40, gap: 14 }, back: { fontSize: 14, fontWeight: "900" }, headRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 14 }, eyebrow: { fontSize: 10, letterSpacing: 1.3, fontWeight: "900" }, title: { fontFamily: "Georgia", fontSize: 28, lineHeight: 36, fontWeight: "700" }, progressPill: { borderWidth: 1, borderRadius: 15, alignItems: "center", paddingHorizontal: 11, paddingVertical: 8 }, progressNumber: { fontSize: 16, fontWeight: "900" }, progressLabel: { fontSize: 10 }, roundCard: { borderWidth: 1, borderRadius: 22, padding: 17, gap: 12 }, meta: { flexDirection: "row", justifyContent: "space-between", gap: 8, alignItems: "center" }, save: { borderWidth: 1, borderRadius: 999, paddingHorizontal: 11, paddingVertical: 7 }, saveText: { fontSize: 11, fontWeight: "900" }, activityTitle: { fontSize: 20, lineHeight: 27, fontWeight: "900" }, stack: { gap: 10 }, passage: { borderWidth: 1, borderRadius: 16, padding: 14, gap: 6 }, passageTitle: { fontSize: 15, fontWeight: "900" }, body: { fontSize: 14, lineHeight: 21 }, prompt: { fontSize: 16, lineHeight: 23, fontWeight: "900" }, helper: { fontSize: 12, lineHeight: 17 }, option: { borderWidth: 1, borderRadius: 15, padding: 13 }, optionText: { fontSize: 14, lineHeight: 20, fontWeight: "700" }, feedback: { borderWidth: 1, borderRadius: 15, padding: 14, gap: 5 }, feedbackTitle: { fontSize: 14, fontWeight: "900" }, input: { minHeight: 116, borderWidth: 1, borderRadius: 15, padding: 13, textAlignVertical: "top", fontSize: 14, lineHeight: 20 }, compactInput: { minHeight: 52, borderWidth: 1, borderRadius: 15, paddingHorizontal: 13, fontSize: 16 }, nextButton: { minHeight: 53, borderRadius: 16, alignItems: "center", justifyContent: "center" }, nextText: { fontSize: 15, fontWeight: "900" }, freshButton: { minHeight: 47, borderWidth: 1, borderRadius: 15, alignItems: "center", justifyContent: "center" }, freshText: { fontSize: 14, fontWeight: "900" }, snapshot: { borderWidth: 1, borderRadius: 20, padding: 15, gap: 12 }, snapshotRow: { flexDirection: "row", justifyContent: "space-between", gap: 7 }, statValue: { fontSize: 20, fontWeight: "900" }, statLabel: { fontSize: 10, marginTop: 1, maxWidth: 63 }, reviewLink: { fontSize: 13, fontWeight: "900", textAlign: "center" }, link: { fontSize: 14, fontWeight: "900", marginTop: 16 }, pressed: { opacity: 0.82, transform: [{ scale: 0.98 }] },
});
