...
export default function AcademicHomeScreen() {
  ...
  const beginSharedRound = () => {
    ...
    router.push({ pathname: "/university-topics", params: { packId: primaryCoursePack.id } } as never);
  };
  const reviewSaved = () => {
    haptic.light();
    if (nursing) router.push("/bookmark-review" as never);
    else if (primaryCoursePack) router.push({ pathname: "/university-topics", params: { packId: primaryCoursePack.id } } as never);
  };
  const primaryActionLabel = requiresLocalInstall && !packInstalled ? "Add pack for offline study" : nursing ? "Start learning round" : "Start varied topic session";

  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Pressable onPress={() => { haptic.light(); router.push("/university-portal" as never); }} accessibilityRole="button"><Text style={[styles.portalBack, { color: colors.primary }]}>‹ University portal</Text></Pressable>
