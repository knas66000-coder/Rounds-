...
export default function StudyScreen() {
  ...
  const activePortal = localProfile ? portalForProgram(localProfile.program) : null;
  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <FlatList
        ...
        showsVerticalScrollIndicator={false}
        ListHeaderComponent={<View style={styles.homeHeader}><View style={styles.headerRow}><View style={styles.headerCopy}><Text style={[styles.eyebrow, { color: colors.primary }]}>STUDY SPACE</Text><Text style={[styles.title, { color: colors.foreground }]}>Pick up where you left off.</Text></View><Pressable onPress={() => { haptic.light(); router.push("/settings" as never); }} accessibilityRole="button" accessibilityLabel="Open settings" style={({ pressed }) => [styles.settingsButton, { backgroundColor: colors.surface, borderColor: colors.border }, pressed && styles.pressed]}><Text style={[styles.settingsGlyph, { color: colors.primary }]}>•••</Text></Pressable></View><Text style={[styles.sub, { color: colors.muted }]}>Short, focused tools built for a mobile study session.</Text><Pressable onPress={() => { haptic.light(); router.push("/learning-portals" as never); }} accessibilityRole="button" accessibilityLabel="Choose University or High School portal" style={({ pressed }) => [styles.localProfileCard, { borderColor: colors.border, backgroundColor: colors.surface }, pressed && styles.pressed]}><View><Text style={[styles.localProfileLabel, { color: colors.primary }]}>{activePortal ? `${activePortal === "university" ? "UNIVERSITY" : "HIGH SCHOOL"} PORTAL` : "CHOOSE A STUDY PORTAL"}</Text><Text style={[styles.localProfileTitle, { color: colors.foreground }]}>{localProfile ? localProfile.institutionName : localProfileReady ? "University or High School" : "Preparing private study"}</Text><Text style={[styles.localProfileText, { color: colors.muted }]}>{localProfile ? "Open the separate University or High School portal. Your current private path stays on this device." : "Select University or High School first, then choose only the matching program or subject. Create an online profile only when you choose Community."}</Text></View><Text style={[styles.localProfileArrow, { color: colors.primary }]}>›</Text></Pressable><Pressable onPress={() => { haptic.medium(); router.push("/mock-exam" as never); }} accessibilityRole="button" style={({ pressed }) => [styles.mockCard, { backgroundColor: colors.primary }, pressed && styles.pressed]}><View><Text style={[styles.mockLabel, { color: colors.background }]}>TIMED SIMULATION</Text><Text style={[styles.mockTitle, { color: colors.background }]}>Mock Exam</Text><Text style={[styles.mockBody, { color: colors.background }]}>25 questions · 60 minutes · study simulation</Text></View><Text style={[styles.mockArrow, { color: colors.background }]}>›</Text></Pressable><Text style={[styles.sectionLabel, { color: colors.muted }]}>LEARNING TOOLS</Text></View>}
...
const styles = StyleSheet.create({
  ...
  localProfileCard: { minHeight: 92, borderWidth: 1, borderRadius: 20, padding: 15, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 14 },
  localProfileLabel: { fontSize: 9, letterSpacing: 1.2, fontWeight: "900" },
  localProfileTitle: { fontSize: 15, fontWeight: "900", marginTop: 4 },
  localProfileText: { fontSize: 12, lineHeight: 17, marginTop: 3, maxWidth: 285 },
  localProfileArrow: { fontSize: 29, fontWeight: "300" },
