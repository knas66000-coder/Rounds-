...
export default function CourseActivityScreen() {
  ...
  return <ScreenContainer className="px-5" edges={["top", "left", "right"]}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Pressable onPress={() => router.back()} accessibilityRole="button"><Text style={[styles.back, { color: colors.primary }]}>‹ Course packs</Text></Pressable><Text style={[styles.eyebrow, { color: colors.primary }]}>{activity.eyebrow}</Text><Text style={[styles.title, { color: colors.foreground }]}>{activity.title}</Text>{activity.kind === "evidence_reading" ? <EvidenceActivity activity={activity} selectedOption={selectedOption} colors={colors} onChoose={chooseOption} /> : activity.kind === "writing_planner" ? <WritingPlanner activity={activity} drafts={drafts} saved={saved} colors={colors} onChange={(id, value) => setDrafts((current) => ({ ...current, [id]: value }))} onSave={() => void saveDraft()} /> : activity.kind === "worked_calculation" ? <CalculationActivity activity={activity} answer={calculationAnswer} feedback={calculationFeedback} colors={colors} onChange={setCalculationAnswer} onSubmit={submitCalculation} /> : activity.kind === "scenario" ? <ScenarioActivity activity={activity} selectedOption={selectedOption} colors={colors} onChoose={chooseOption} /> : <LogicTraceActivity activity={activity} selectedOption={selectedOption} colors={colors} onChoose={chooseOption} />}{saved ? <View style={[styles.saved, { borderColor: colors.success, backgroundColor: colors.surface }]}><Text style={[styles.savedTitle, { color: colors.success }]}>Saved locally</Text><Text style={[styles.savedText, { color: colors.muted }]}>Your progress is stored on this device. This starter activity is not an institutional grade.</Text></View> : null}</ScrollView></ScreenContainer>;
}
...
function CalculationActivity({ activity, answer, feedback, colors, onChange, onSubmit }: { activity: Extract<ReturnType<typeof starterActivityFor>, { kind: "worked_calculation" }>; answer: string; feedback: ReturnType<typeof evaluateCalculationAnswer> | null; colors: ReturnType<typeof useColors>; onChange: (value: string) => void; onSubmit: () => void }) {
  const success = feedback?.outcome === "correct";
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.surface }]}><Text style={[styles.passageText, { color: colors.muted }]}>{activity.scenario}</Text>{activity.safetyNote ? <Text style={[styles.fieldHelper, { color: colors.muted }]}>{activity.safetyNote}</Text> : null}</View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text><TextInput value={answer} onChangeText={onChange} editable={!feedback} keyboardType="decimal-pad" placeholder={`Enter a number${activity.unit ? ` (${activity.unit.trim()})` : ""}`} placeholderTextColor={colors.muted} style={[styles.compactInput, { borderColor: colors.border, backgroundColor: colors.background, color: colors.foreground }]} accessibilityLabel="Calculation answer" /><Pressable onPress={onSubmit} disabled={Boolean(feedback)} accessibilityRole="button" style={({ pressed }) => [styles.saveButton, { backgroundColor: colors.primary }, (pressed || Boolean(feedback)) && styles.pressed]}><Text style={[styles.saveButtonText, { color: colors.background }]}>{feedback ? "Answer checked" : "Check answer"}</Text></Pressable>{feedback ? <View style={[styles.feedback, { borderColor: success ? colors.success : colors.warning, backgroundColor: colors.surface }]}><Text style={[styles.feedbackTitle, { color: success ? colors.success : colors.warning }]}>{success ? "Calculation matched" : "Review the steps"}</Text><Text style={[styles.feedbackText, { color: colors.muted }]}>{feedback.feedback}</Text></View> : null}</View>;
}

function LogicTraceActivity({ activity, selectedOption, colors, onChoose }: { activity: Extract<ReturnType<typeof starterActivityFor>, { kind: "logic_trace" }>; selectedOption: string | null; colors: ReturnType<typeof useColors>; onChoose: (option: string) => void }) {
  const result = selectedOption ? evaluateLogicTraceAnswer(selectedOption, activity) : null;
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.surface }]}><Text style={[styles.passageText, { color: colors.muted }]}>{activity.scenario}</Text></View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text>{activity.options.map((option) => <Pressable key={option} disabled={Boolean(selectedOption)} onPress={() => onChoose(option)} accessibilityRole="button" style={({ pressed }) => [styles.option, { borderColor: selectedOption === option ? (result?.outcome === "correct" ? colors.success : colors.error) : colors.border, backgroundColor: selectedOption === option ? colors.surface : colors.background }, pressed && !selectedOption && styles.pressed]}><Text style={[styles.optionText, { color: colors.foreground }]}>{option}</Text></Pressable>)}{result ? <View style={[styles.feedback, { borderColor: result.outcome === "correct" ? colors.success : colors.warning, backgroundColor: colors.surface }]}><Text style={[styles.feedbackTitle, { color: result.outcome === "correct" ? colors.success : colors.warning }]}>{result.outcome === "correct" ? "Logic matched" : "Review the rule order"}</Text><Text style={[styles.feedbackText, { color: colors.muted }]}>{result.feedback}</Text></View> : null}</View>;
}

function ScenarioActivity({ activity, selectedOption, colors, onChoose }: { activity: Extract<ReturnType<typeof starterActivityFor>, { kind: "scenario" }>; selectedOption: string | null; colors: ReturnType<typeof useColors>; onChoose: (option: string) => void }) {
  const result = selectedOption ? evaluateScenarioDecision(selectedOption, activity) : null;
  return <View style={styles.stack}><View style={[styles.passage, { borderColor: colors.border, backgroundColor: colors.surface }]}><Text style={[styles.passageText, { color: colors.muted }]}>{activity.scenario}</Text>{activity.boundaryNote ? <Text style={[styles.fieldHelper, { color: colors.muted }]}>{activity.boundaryNote}</Text> : null}</View><Text style={[styles.prompt, { color: colors.foreground }]}>{activity.prompt}</Text>{activity.options.map((option) => <Pressable key={option} disabled={Boolean(selectedOption)} onPress={() => onChoose(option)} accessibilityRole="button" style={({ pressed }) => [styles.option, { borderColor: selectedOption === option ? (result?.outcome === "correct" ? colors.success : colors.error) : colors.border, backgroundColor: selectedOption === option ? colors.surface : colors.background }, pressed && !selectedOption && styles.pressed]}><Text style={[styles.optionText, { color: colors.foreground }]}>{option}</Text></Pressable>)}{result ? <View style={[styles.feedback, { borderColor: result.outcome === "correct" ? colors.success : colors.warning, backgroundColor: colors.surface }]}><Text style={[styles.feedbackTitle, { color: result.outcome === "correct" ? colors.success : colors.warning }]}>{result.outcome === "correct" ? "Decision matched" : "Review the scenario"}</Text><Text style={[styles.feedbackText, { color: colors.muted }]}>{result.feedback}</Text></View> : null}</View>;
}

const styles = StyleSheet.create({
  content: { paddingTop: 19, paddingBottom: 40, gap: 12 }, back: { fontSize: 14, fontWeight: "900", marginBottom: 3 }, eyebrow: { fontSize: 10, letterSpacing: 1.4, fontWeight: "900" }, title: { fontFamily: "Georgia", fontSize: 30, lineHeight: 38, fontWeight: "700" }, body: { fontSize: 14, lineHeight: 21, marginTop: 8 }, link: { fontSize: 14, fontWeight: "900", marginTop: 16 }, stack: { gap: 11 }, passage: { borderWidth: 1, borderRadius: 18, padding: 16, gap: 8 }, passageTitle: { fontSize: 16, fontWeight: "900" }, passageText: { fontSize: 14, lineHeight: 22 }, prompt: { fontSize: 17, lineHeight: 24, fontWeight: "900", marginTop: 3 }, option: { borderWidth: 1, borderRadius: 16, padding: 14 }, optionText: { fontSize: 14, lineHeight: 20, fontWeight: "700" }, feedback: { borderWidth: 1, borderRadius: 16, padding: 15, gap: 5 }, feedbackTitle: { fontSize: 14, fontWeight: "900" }, feedbackText: { fontSize: 13, lineHeight: 20 }, field: { gap: 5 }, fieldLabel: { fontSize: 16, fontWeight: "900" }, fieldHelper: { fontSize: 12, lineHeight: 17 }, input: { minHeight: 106, borderWidth: 1, borderRadius: 14, padding: 12, textAlignVertical: "top", fontSize: 14, lineHeight: 20 }, compactInput: { minHeight: 52, borderWidth: 1, borderRadius: 14, paddingHorizontal: 12, fontSize: 16 }, saveButton: { minHeight: 52, borderRadius: 16, alignItems: "center", justifyContent: "center", marginTop: 3 }, saveButtonText: { fontSize: 15, fontWeight: "900" }, saved: { borderWidth: 1, borderRadius: 16, padding: 14, gap: 4 }, savedTitle: { fontSize: 14, fontWeight: "900" }, savedText: { fontSize: 12, lineHeight: 18 }, pressed: { opacity: 0.82, transform: [{ scale: 0.98 }] },
});
