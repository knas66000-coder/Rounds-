...
## Academic profile and personal program home

Rounds uses one authentication system for the entire installed application. Because account identity is completed by the secure sign-in provider, the academic details are collected immediately **after the first successful sign-in** rather than inside the external identity form. The required profile asks for the learner’s university or college and their academic program. It is a private academic preference, not an institutional verification or an official enrolment record.

The learner completes a one-handed, portrait onboarding screen: **University or college → Program → Continue to my study pack**. Program choices begin with Nursing, Engineering, Computing, Business, Natural Sciences, Education, Social Sciences, and University Foundation Year. The completed profile is stored per authenticated learner, can be updated in Settings, and is never exposed through Community.

After completion, Rounds opens the learner’s **personal program home**. Nursing opens the active Nursing practice pack and retains access to the current NCLEX tools. Other programs receive a dedicated program pack page instead of being sent to Nursing; it identifies their pack, explains the planned course-unit path, and does not falsely imply that Nursing questions are engineering or business content. As future downloadable packs arrive, the same destination activates the correct installed units automatically.
