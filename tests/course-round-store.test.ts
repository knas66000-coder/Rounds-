...
describe("Rounds shared learning-round state", () => {
  it("keeps queues subject-specific and no-repeat by activity identity", () => {
    ...
    expect(foundation.map(courseRoundActivityId)).toEqual(["university-foundation-year:academic-writing", "university-foundation-year:digital-literacy"]);
    expect(new Set(foundation.map(courseRoundActivityId)).size).toBe(foundation.length);
    expect(activitiesForCourseRound("computing-foundations", state).map(courseRoundActivityId)).toEqual(["computing-foundations:computing-requirements"]);
  });
